...
describe('RegexBuilder Tests', () => {
    describe('Create variable', () => {
        test('integer', () => {
            const result = RegexBuilder.createVariable('varName', 'integer');
            expect(result).toBe('(?<varName>\\d+)');
        });
        test('decimal', () => {
            const result = RegexBuilder.createVariable('varName', 'decimal');
            expect(result).toBe('(?<varName>\\d+(\\.\\d+)?)');
        });
        test('word', () => {
            const result = RegexBuilder.createVariable('varName', 'word');
            expect(result).toBe('(?<varName>\\w)');
        });
        test('words', () => {
            const result = RegexBuilder.createVariable('varName', 'words');
            expect(result).toBe('(?<varName>\\w+)');
        });
        test('specific values', () => {
            const result = RegexBuilder.createVariable('varName', ['val1', 'val2']);
            expect(result).toBe('(?<varName>(val1|val2))');
        });
    });

    test('Use variable', () => {
        const result = RegexBuilder.useVariable('varName');
